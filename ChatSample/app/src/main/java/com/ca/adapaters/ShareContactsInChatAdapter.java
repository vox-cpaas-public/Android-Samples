...
import com.ca.chatsample.ShareContactsInChatActivity;
...
public class ShareContactsInChatAdapter extends CursorAdapter {
	private Context context;
	public  ArrayList<Boolean> itemChecked = new ArrayList<Boolean>();

	public ShareContactsInChatAdapter(Context context, Cursor c, int flags) {
	...
	public void bindView(View convertView, final Context context, Cursor cursor) {
		try {
            ...
            String picid = cursor.getString(cursor.getColumnIndexOrThrow(CSDbFields.KEY_CONTACT_ID));
			String myrawnumber = cursor.getString(cursor.getColumnIndexOrThrow(CSDbFields.KEY_CONTACT_RAW_NUMBER));

           secondary.setText(number);

			new ImageDownloaderTask(image).execute("native",picid);


			if(ShareContactsInChatActivity.rawnumbers.contains(myrawnumber)) {
